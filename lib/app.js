...
App.prototype.bundle_meteor = function ( /* server_url, */ done) {
  ...
  spawn('meteor' + (this.$.env.os.is_windows ? '.bat' : ''), [
    ...
    env: {
      METEOR_SETTINGS: process.env.ELECTRIFY_SETTINGS_FILE
    }
  }
  ).on('exit', function () {

    var afterBuild = function () {
      console.log("bundled_dir: ", bundled_dir)
      console.log("electrify_app_dir: ", electrify_app_dir)
      console.log("tmp_dir: ", tmp_dir)
      console.log("self.$.env.meteor.server_modules: ", self.$.env.meteor.server_modules)
      console.log("programs_server_dir: ", programs_server_dir)
...
App.prototype.ensure_deps = function (done) {
  ...
  if (this.$.env.is_development_mode) {
    ...
    var config = require(pkg_path);
    var pkg_version = _.get(config, "devDependencies['meteor-electrify']") || _.get(config, "dependencies['meteor-electrify']");
    var version = this.$.env.version;

    // check and warn users about electrify versions mismatch
    if (pkg_version != version) {
      this.log.warn([
        "{{IMPORTANT}} Your local meteor-electrify npm package version (",
        pkg_version,
        ") must match version (",
        version,
