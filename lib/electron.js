...
Electron.prototype.package = function (packager_options, done) {
  ...
  console.log(this.$.env.is_development_mode)
    ...
    args.afterCopy = [
      ...
      function (buildPath, electronVersion, platform, arch, callback) {
        ...
        if (fs.lstatSync(electrifyNodeModulesPath).isSymbolicLink()) {
          console.log("is syunm l;inalsdhjflaksjdflasdhgajkhgasdkljfkajghasdjkfgh")
  ...
  packager(args, function (err) {
    ...
    console.log("self.$.env.app.dist: ", self.$.env.app.dist)
    console.log("tmp_package_dir: ", tmp_package_dir)
